...
class FeatureEngineer:
    ...
    def add_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Speed Index用エイリアス（advanced_features.pyがtrack_condition_codeを参照）
        ...
        # v18: title パースは不確実なため、grade_code と kyoso_joken_code を優先する
        def parse_class_level(row):
            title = str(row.get('title', ''))
            grade = str(row.get('grade_code', ' '))
            joken = str(row.get('kyoso_joken_code', ' '))
            shubetsu = str(row.get('kyoso_shubetsu_code', ' '))
            
            # 1. 重賞 (G1, G2, G3)
            if grade == 'A': return 9 # G1
            if grade == 'B': return 8 # G2
            if grade == 'C': return 7 # G3
            if grade == 'D' or grade == 'E': return 6 # L or OP
            
            # 2. 条件戦 (joken_code)
            if joken == '705' or joken == '706': return 5 # 3勝クラス
            if joken == '703' or joken == '704': return 4 # 2勝クラス
            if joken == '701' or joken == '702': return 3 # 1勝クラス
            if joken == '000': return 2 # 未勝利
            if joken == '005': return 1 # 新馬
            
            # 3. title によるフォールバック (従来のロジック)
            ...
            # 障害競走
            if shubetsu == '18': return 5 # 暫定
            
            return 5 # 不明時は5(3勝クラス相当)とする

        df['class_level'] = df.apply(parse_class_level, axis=1).astype('int8')
        ...
        # 8. 着差 (Time Difference) の計算
        # 各レースの勝ちタイム（最小タイム）との差分
        if 'time' in df.columns:
            # groupby().transform('min') でレースごとの勝ちタイムを全行に付与
            # timeがNaN（競走中止など）の場合はtime_diffもNaNになる
            win_times = df.groupby('race_id')['time'].transform('min')
            df['time_diff'] = df['time'] - win_times
        else:
            df['time_diff'] = np.nan
