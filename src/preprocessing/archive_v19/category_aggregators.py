...
- 2025-12-19 v12: P1対応 - ベイズ平滑化導入 (min_samples + 全体平均へ縮約)
"""
...
# P1: ベイズ平滑化パラメータ
# smoothed_rate = (wins + PRIOR_SAMPLES * global_avg) / (races + PRIOR_SAMPLES)
PRIOR_SAMPLES = 30.0  # 強めに設定（専門家のアドバイスに基づき正則化を強化）
GLOBAL_WIN_RATE = 0.07  # 全体平均勝率（約14頭立て × 1/14 ≈ 7%）
GLOBAL_TOP3_RATE = 0.21  # 全体平均複勝率（約3/14 ≈ 21%）
...
class CategoryAggregator:
    ...
    def aggregate(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, col in enumerate(targets):
            logger.info(f"[{i+1}/{len(targets)}] Basic Aggregation for {col}...")
        ...
        if 'surface' in df.columns and 'sire_id' in df.columns:
             logger.info("Context Aggregation: sire x surface...")
        ...
        if 'trainer_id' in df.columns and 'jockey_id' in df.columns:
            logger.info("Context Aggregation: jockey x trainer...")
    ...
    def _aggregate_basic(self, df: pd.DataFrame, col: str) -> pd.DataFrame:
        ...
        # P1: ベイズ平滑化 (wins + m*p) / (races + m)
        cum_races_safe = df['cum_races'].fillna(0)
        cum_wins_safe = df['cum_wins'].fillna(0)
        cum_top3_safe = df['cum_top3'].fillna(0)
        
        df[f'{col}_win_rate'] = (cum_wins_safe + PRIOR_SAMPLES * GLOBAL_WIN_RATE) / (cum_races_safe + PRIOR_SAMPLES)
        df[f'{col}_top3_rate'] = (cum_top3_safe + PRIOR_SAMPLES * GLOBAL_TOP3_RATE) / (cum_races_safe + PRIOR_SAMPLES)
    ...
    def _aggregate_context(self, df: pd.DataFrame, target_col: str, cond_col: str, suffix: str) -> pd.DataFrame:
        ...
        df[f'{feature_prefix}_win_rate'] = (cum_wins_safe + PRIOR_SAMPLES * GLOBAL_WIN_RATE) / (cum_races_safe + PRIOR_SAMPLES)
        df[f'{feature_prefix}_top3_rate'] = (cum_top3_safe + PRIOR_SAMPLES * GLOBAL_TOP3_RATE) / (cum_races_safe + PRIOR_SAMPLES)
