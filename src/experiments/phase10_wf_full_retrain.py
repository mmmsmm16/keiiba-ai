...
def train_and_predict_wf(df, noleak_pq_path):
    ...
    for m in range(1, 13):
        ...
        if test_data.empty:
            logger.warning(f"No data for 2025-{m:02d}, skipping.")
            continue
            
        # Train Data: All History before this month
        cutoff_date = datetime(2025, m, 1)
        train_df = df[df['date'] < cutoff_date].copy()
        
        # Features: STRICT ALLOWLIST (No Price, No Rank, No Time)
        def is_allowed(c):
        ...
        # Optimized for Speed/Convergence (Based on Phase 9)
        params = {
            'objective': 'binary',
        ...
        model = lgb.train(
            params,
            lgb_train,
            num_boost_round=40   # Converges quickly
        )
        ...
        results.append(test_data[['race_id', 'horse_number', 'odds_tminus10m', 'pred_prob', 'pred_logit', 'base_logit']])
...
def main():
    ...
    eval_df = pd.merge(wf_preds, results, on=['race_id', 'horse_number'], how='inner')
    ...
    logger.info(f"WF 2025 Full LogLoss: {loss:.4f}")
    ...
    if not bets.empty:
        ...
        logger.info(f"WF 2025 Full ROI (EV>1.0): {roi:.2f}% ({len(bets)} bets)")
    else:
