import pandas as pd
import logging
from .loader import NarDataLoader
...
class BloodlineFeatureEngineer:
    ...
    """
    def __init__(self, data_loader: NarDataLoader = None):
        """
        Args:
            data_loader: NarDataLoaderのインスタンス。
                         指定がない場合は新規に作成しますが、
                         パフォーマンスのため、既存のインスタンスを渡すことを推奨します。
        """
        self.loader = data_loader if data_loader else NarDataLoader()
