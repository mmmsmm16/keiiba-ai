...
# プロジェクトルート (src の親ディレクトリ) をパスに追加
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '../..'))
if project_root not in sys.path:
    sys.path.insert(0, project_root)

from src.nar.loader import NarDataLoader
from src.nar.cleansing import DataCleanser
from src.nar.feature_engineering import FeatureEngineer
from src.nar.aggregators import HistoryAggregator
from src.nar.category_aggregators import CategoryAggregator
from src.nar.dataset import DatasetSplitter
from src.nar.advanced_features import AdvancedFeatureEngineer
from src.nar.disadvantage_detector import DisadvantageDetector
from src.nar.relative_features import RelativeFeatureEngineer
from src.nar.experience_features import ExperienceFeatureEngineer
from src.nar.race_level_features import RaceLevelFeatureEngineer
from src.nar.validation_utils import validate_data_quality
...
def main():
    ...
    parser.add_argument('--resume', action='store_true', help='Resume from Step 5 checkpoint')
    ...
    try:
        ...
        if args.resume and os.path.exists(checkpoint_path) and not args.incremental:
            ...
            # 1. データのロード (通常モード)
            logger.info(f"Step 1: データのロード (NAR/PC-KEIBA)")
        ...
        from src.nar.bloodline_features import BloodlineFeatureEngineer
        ...
        from src.nar.realtime_features import RealTimeFeatureEngineer
        ...
        if args.use_embedding_features:
            from src.nar.embedding_features import EmbeddingFeatureEngineer
        ...
        from src.nar.opposition_features import OppositionFeatureEngineer
        ...
        from src.nar.validation_utils import check_feature_leak
