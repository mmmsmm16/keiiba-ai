...
def run_grid_search(pred_df, payout_map, fold_name):
    ...
    threshold_conditions = [
        ...
        {'name': 'score_diff_02', 'min_odds': None, 'max_odds': None, 'min_score_diff': 0.02},
        {'name': 'score_diff_05', 'min_odds': None, 'max_odds': None, 'min_score_diff': 0.05},
    ]
    ...
    for cond in threshold_conditions:
        ...
        if cond['min_odds'] is not None or cond['max_odds'] is not None:
            def filter_by_top1_odds(group, min_o, max_o):
                ...
                if top1.empty:
                    return False
                odds = top1['odds'].values[0] if 'odds' in top1.columns else None
        ...
        for name, bet_type, top_n in base_strategies:
            ...
            if result['roi'] > 80:  # 注目すべき結果のみ表示
                logger.info(f"  ★ {strategy_name}: ROI={result['roi']:.1f}%, Hit={result['hit_rate']:.2%}")
    
    return results

def main():
    parser = argparse.ArgumentParser(description='時系列CV戦略グリッドサーチ (v23回帰)')
    ...
    logger.info("=" * 60)
    logger.info("時系列CV 戦略グリッドサーチ (v23回帰モデル)")
    logger.info("=" * 60)
    ...
    logger.info(f"\n一貫して良好な戦略 (全Fold ROI >= 70%): {len(consistent_strategies)} 件")
    for s in sorted(consistent_strategies, key=lambda x: -x['avg_roi']):
        logger.info(f"  {s['strategy']}: 平均={s['avg_roi']:.1f}%, 最小={s['min_roi']:.1f}%, 最大={s['max_roi']:.1f}%, 的中率={s['avg_hit_rate']:.2%}")
