"""
LambdaRank + Optuna HPO (Batch 4) with odds_10min-weighted loss
==============================================================
Uses odds_10min to upweight correct top finishes, while keeping ranking objective.
"""
import os
import sys
import json
...
OUTPUT_DIR = "models/experiments/exp_lambdarank_v12_batch4_optuna_odds10_weighted"
N_TRIALS = 30

ODDS_COL = "odds_10min"
ODDS_FALLBACK_COL = "odds"
ODDS_CLIP_MIN = 1.01
WEIGHT_MODE = "log1p"  # "log1p", "sqrt", or "pow"
...
def build_sample_weights(df, odds_col, fallback_col, alpha, clip_min, clip_max, weight_mode):
    """Build sample weights using odds for positive relevance only."""
    if odds_col in df.columns:
        odds_raw = df[odds_col]
    elif fallback_col in df.columns:
        odds_raw = df[fallback_col]
    else:
        return np.ones(len(df), dtype=np.float32)

    odds = pd.to_numeric(odds_raw, errors='coerce').clip(lower=clip_min, upper=clip_max)

    if weight_mode == "log1p":
        odds_score = np.log1p(odds)
    elif weight_mode == "sqrt":
        odds_score = np.sqrt(odds)
    elif weight_mode == "pow":
        odds_score = odds
    else:
        raise ValueError(f"Unknown weight_mode: {weight_mode}")

    odds_score = odds_score.fillna(0.0)

    rel = df['relevance'].clip(lower=0) / 3.0
    weights = 1.0 + (rel * (odds_score ** alpha))
    return weights.astype(np.float32).values
...
def create_lgb_dataset(df, feature_cols):
    """Create LightGBM dataset arrays for ranking."""
...
def objective(trial, train_df, valid_df, feature_cols):
    ...
    weight_alpha = trial.suggest_float('weight_alpha', 0.2, 1.2)
    weight_clip_max = trial.suggest_float('weight_clip_max', 10.0, 200.0, log=True)
    ...
    w_train = build_sample_weights(
        train_df, ODDS_COL, ODDS_FALLBACK_COL, weight_alpha, ODDS_CLIP_MIN, weight_clip_max, WEIGHT_MODE
    )
    w_valid = build_sample_weights(
        valid_df, ODDS_COL, ODDS_FALLBACK_COL, weight_alpha, ODDS_CLIP_MIN, weight_clip_max, WEIGHT_MODE
    )

    train_data = lgb.Dataset(X_train, label=y_train, group=groups_train, weight=w_train)
    valid_data = lgb.Dataset(X_valid, label=y_valid, group=groups_valid, weight=w_valid, reference=train_data)
...
def main():
    logger.info("=" * 60)
    logger.info("LambdaRank + Optuna HPO (Batch 4) w/ odds_10min weighting")
    logger.info("=" * 60)
    ...
    study = optuna.create_study(direction='maximize', study_name='lambdarank_hpo_odds10_weighted')
    study.optimize(
        lambda trial: objective(trial, train_df, valid_df, feature_cols),
        n_trials=N_TRIALS,
    ...
    weight_params = {
        'weight_alpha': best_params.pop('weight_alpha'),
        'weight_clip_max': best_params.pop('weight_clip_max')
    }
    ...
    w_train = build_sample_weights(
        train_df, ODDS_COL, ODDS_FALLBACK_COL, weight_params['weight_alpha'],
        ODDS_CLIP_MIN, weight_params['weight_clip_max'], WEIGHT_MODE
    )
    w_valid = build_sample_weights(
        valid_df, ODDS_COL, ODDS_FALLBACK_COL, weight_params['weight_alpha'],
        ODDS_CLIP_MIN, weight_params['weight_clip_max'], WEIGHT_MODE
    )
    ...
    payout_col = ODDS_FALLBACK_COL if ODDS_FALLBACK_COL in test_df.columns else ODDS_COL
    total_bet = len(top1_bets) * 100
    returns = top1_bets[top1_bets['is_win'] == 1][payout_col].sum() * 100
    roi = returns / total_bet * 100 if total_bet > 0 else 0

    logger.info(f"\nWin ROI (Bet Top 1): {roi:.2f}% (payout: {payout_col})")
    logger.info(f"Total Bets: {len(top1_bets)}, Wins: {len(top1_bets[top1_bets['is_win'] == 1])}")
