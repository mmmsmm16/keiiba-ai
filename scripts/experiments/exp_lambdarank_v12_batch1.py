...
"""
LambdaRank Experiment Script (v12 Batch 1)
==========================================
Trains a LightGBM LambdaRank model using preprocessed_data_v12.parquet.
Includes Mining Index and Track Aptitude features.
"""
import os
import sys
import logging
...
TARGET_PATH = "data/temp_t2/T2_targets.parquet" # Fallback if needed
OUTPUT_DIR = "models/experiments/exp_lambdarank_v12_batch1"
...
def load_data():
    ...
    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(f"{DATA_PATH} not found. Please run scripts/rebuild_v12.py first.")
        
    df = pd.read_parquet(DATA_PATH)
    
    # Merge targets if rank is missing (should not be needed for v12 usually)
...
def prepare_lgb_dataset(df, feature_cols):
    ...
    for c in X.columns:
        ...
        # LightGBM handles NaN, but fill with -999 for safety if not category? 
        # Actually LightGBM handles NaN natively.
...
def evaluate_ndcg(model, X, y, race_ids, k=3):
    ...
    ndcg_list = []
    # GroupBy is slow for large DF, but ok for evaluation
...
def run_experiment():
    logger.info("=" * 60)
    logger.info("LambdaRank Experiment (v12 Batch 1)")
    logger.info("=" * 60)
    ...
    # LEAKAGE COLS (Do not use for training!)
    leakage_cols = [
        ...
        # Odds Features (Explicit exclusion per user request)
        ...
        # Batch 2 Ineffective (keeping is_transported and is_away_jockey)
