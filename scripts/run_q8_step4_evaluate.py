...
def main():
    ...
    # Load Odds if not present
    if 'odds' not in df.columns:
        odds_path = f"data/temp_q1/year_{EVAL_YEAR}.parquet"
        if os.path.exists(odds_path):
            logger.info(f"Loading Odds from {odds_path}")
            # Note: loader saves as 'odds'
            df_odds = pd.read_parquet(odds_path, columns=['race_id', 'horse_number', 'odds'])
            # Cast for merge
            # race_id might be object in df?
            # df comes from Q8_targets which has race_id as object usually.
            # df_odds likely has it as object too.
            eval_df = pd.merge(eval_df, df_odds, on=['race_id', 'horse_number'], how='left')
        else:
             logger.warning("Odds file not found. Skipping ROI.")
             
    if 'odds' in eval_df.columns:
        # Strategy: Bet on highest prob
        # eval_df has ['race_id', 'horse_number', 'rank', 'y_prob', 'odds']
        
        # Rank by prob desc
        eval_df['pred_rank'] = eval_df.groupby('race_id')['y_prob'].rank(ascending=False, method='first')
        
        # Bet on Rank 1
        bets = eval_df[eval_df['pred_rank'] == 1]
        
        # Filter out bets where odds are 0 (Missing Data)
        # 40% of winners have 0 odds in current data, which kills ROI.
        # We simulate ROI only on "Valid Bets" (where we know the price).
        ...
        return_amount = (hits['odds'] * 100).sum()
        ...
        logger.info(f"Return: {return_amount:.0f} / {cost} = {roi*100:.2f}%")
