...
def create_bias_notebook():
    nb = nbf.v4.new_notebook()
    
    # Markdown - Title
    nb.cells.append(nbf.v4.new_markdown_cell(
        "# 地方競馬（NAR）馬場バイアス導入モデル (16モデル)\n\n"
        "当日・同会場の「直前までのレース結果」を集計し、リアルタイムな馬場バイアス（内枠有利・外枠有利・逃げ有利など）を特徴量として導入します。\n\n"
        "### 本モデルで検証する仮説\n"
        "- **仮説**: 地方競馬は砂の補充や天候により、日によって「インコースが全く伸びない」「前が止まらない」などの極端なバイアスが発生する。\n"
        "- **実装**: 当日の当該レース発走前までの集計値を特徴量 (`track_bias_inner_win_rate` 等) としてモデルに与える。"
    ))
    ...
    nb.cells.append(nbf.v4.new_code_cell(
        ...
        "]\n\n"
        "features = list(set(baseline_features + advanced_features + phase9_features + track_bias_features))\n\n"
        "# カテゴリ処理\n"
    ...
    nb.cells.append(nbf.v4.new_code_cell(
        ...
        "for i, col in enumerate(track_bias_features):\n"
        "    plt.subplot(1, 3, i+1)\n"
    ...
    # Save
    path = '/workspace/notebooks/nar/16_nar_track_bias_model.ipynb'
    with open(path, 'w', encoding='utf-8') as f:
        nbf.write(nb, f)
    print(f'Notebook created at {path}')

if __name__ == '__main__':
    create_bias_notebook()
