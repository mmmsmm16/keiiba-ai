...
def main():
    logger.info("🚀 Starting M4 Step 2: Training (M4-A-Top3)")
    ...
    if not os.path.exists("data/temp_m4/M4_features.parquet") or not os.path.exists("data/temp_m4/M4_targets.parquet"):
        logger.error("Missing parquet files! Run Step 1 first.")
        return

    df_features = pd.read_parquet("data/temp_m4/M4_features.parquet")
    df_targets = pd.read_parquet("data/temp_m4/M4_targets.parquet")
    ...
    # DatasetSplitter returns pure features in X.
    # We need to map back using index.
