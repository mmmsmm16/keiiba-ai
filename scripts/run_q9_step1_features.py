...
def main():
    logger.info("🚀 Starting Phase Q Step 1 (Iteration 9): Physique & Training Feature Generation")
    
    # Feature Set (Base Q8 + Physique Training)
    FEATURE_BLOCKS = [
        ...
        'interval_aptitude',
        'physique_training' # New
    ]
    
    # We use data/features_q9 as cache dir
    CACHE_DIR = "data/features_q9"
    os.makedirs(CACHE_DIR, exist_ok=True)
    
    # 1. Copy Cache from Q8 to Q9
    logger.info("Copying Q8 cache to Q9...")
    ...
    # 2. Data Loading (Reuse Q8 Temp)
    # We use same temp_q1 base for raw data.
    TEMP_DIR = "data/temp_q9"
    ...
    clean_df[target_cols].to_parquet(f"{TEMP_DIR}/Q9_targets.parquet")
    ...
    # Verify
    if 'weight_impact_score' not in df_features.columns:
         logger.error(f"❌ CRITICAL: {new_block} features missing!")
         raise ValueError("Feature generation failed")
         
    # Save
    out_path = f"{TEMP_DIR}/Q9_features.parquet"
    df_features.to_parquet(out_path)
    logger.info(f"Features saved to {out_path}")
    logger.info("Phase Q Step 1 (Iteration 9) Complete!")
