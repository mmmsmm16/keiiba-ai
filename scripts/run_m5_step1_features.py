...
def main():
    logger.info("🚀 Starting M5 Step 1: Feature Generation (M4-B Set for Ensemble)")
    
    # Config
    START_DATE = "2015-01-01"
    END_DATE = "2024-12-31"
    
    # M4-B Feature Set (Core + Class Stats)
    FEATURE_BLOCKS = [
        ...
        'class_stats' # Key Feature
    ]
    
    os.makedirs("data/temp_m5", exist_ok=True)
    ...
    while current_year <= final_year:
        ...
        # We can reuse M4 temp files if they exist to save time
        # But for M5 let's ensure fresh copy or reuse if exactly same name
        # M4 used data/temp_m4/year_XXXX.parquet
        # To be safe, let's use data/temp_m5/year_XXXX.parquet
        # Or just copy? Loading is fast if cached.
        
        temp_file = f"data/temp_m5/year_{current_year}.parquet"
    ...
    clean_df[target_cols].to_parquet("data/temp_m5/M5_targets.parquet")
    logger.info("Targets saved to data/temp_m5/M5_targets.parquet")
    ...
    df_features.to_parquet("data/temp_m5/M5_features.parquet")
    logger.info("Features saved to data/temp_m5/M5_features.parquet")
    
    logger.info("M5 Step 1 Complete!")
