...
# 2. Find and replace the feature definition cell
for cell in nb.cells:
    if cell.cell_type == 'code' and '# 特徴量リストの定義' in cell.source:
        cell.source = """# 特徴量の定義（高度特徴量の追加）
...
# 3. Update evaluation overview description
for cell in nb.cells:
    if cell.cell_type == 'markdown' and '## 5. 評価' in cell.source:
        cell.source = "## 5. 評価\n\n高度な特徴量を追加したモデルの性能を確認します。重要度では新しく追加した「休養期間」や「人馬相性」が上位に現れるはずです。"

# Write the final notebook
with open(target_nb, 'w', encoding='utf-8') as f:
    nbf.write(nb, f)

print(f"Successfully reconstructed {target_nb}")
