...
def main():
    logger.info("🚀 Starting Phase Q Step 1 (Iteration 6): RunStyle Fit Feature Generation")
    
    # Feature Set (Base Q5 + RunStyle Fit)
    FEATURE_BLOCKS = [
    ...
    # We use data/features_q6 as cache dir
    CACHE_DIR = "data/features_q6"
    os.makedirs(CACHE_DIR, exist_ok=True)
    
    # 1. Copy Cache from Q5 to Q6
    logger.info("Copying Q5 cache to Q6...")
    
    q5_cache = "data/features_q5"
    if os.path.exists(q5_cache):
        import shutil
        import glob
        files = glob.glob(f"{q5_cache}/*.parquet")
        for f in files:
            bn = os.path.basename(f)
            dst = os.path.join(CACHE_DIR, bn)
            if not os.path.exists(dst):
                 shutil.copy2(f, dst)
        logger.info("  -> Copied Q5 cache.")
    else:
        logger.warning("  -> Q5 cache not found! Starting fresh.")
        
    # 2. Data Loading (Reuse Q5 Temp)
    # Target and Raw Data are same.
    TEMP_DIR = "data/temp_q6"
    ...
    clean_df[target_cols].to_parquet(f"{TEMP_DIR}/Q6_targets.parquet")
    ...
    # Verify
    if 'fit_nige_short' not in df_features.columns:
         logger.error(f"❌ CRITICAL: {new_block} features missing!")
         raise ValueError("Feature generation failed")
         
    # Save
    out_path = f"{TEMP_DIR}/Q6_features.parquet"
    df_features.to_parquet(out_path)
    logger.info(f"Features saved to {out_path}")
    logger.info("Phase Q Step 1 (Iteration 6) Complete!")
