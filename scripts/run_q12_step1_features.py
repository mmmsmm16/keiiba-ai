...
def main():
    logger.info("🚀 Starting Phase Q Step 1 (Iteration 12): Sire Aptitude Feature Generation")
    
    # Feature Set (Base Q8 + Sire Aptitude)
    # Note: Excluding Q9, Q10, Q11 features.
    FEATURE_BLOCKS = [
    ...
    # We use data/features_q12 as cache dir
    CACHE_DIR = "data/features_q12"
    os.makedirs(CACHE_DIR, exist_ok=True)
    
    # 1. Copy Cache from Q8 to Q12
    # Q8 is the approved base.
    logger.info("Copying Q8 cache to Q12...")
    ...
    if 'date' in clean_df.columns:
        clean_df[target_cols].to_parquet(f"{TEMP_DIR}/Q12_targets.parquet")
    else:
    ...
    # Verify
    if 'sire_course_win_rate' not in df_features.columns:
         logger.error(f"❌ CRITICAL: {new_block} features missing!")
         raise ValueError("Feature generation failed")
         
    # Save
    out_path = f"{TEMP_DIR}/Q12_features.parquet"
    df_features.to_parquet(out_path)
    logger.info(f"Features saved to {out_path}")
    logger.info("Phase Q Step 1 (Iteration 12) Complete!")
