...
def main():
    logger.info("🚀 Starting M3 Baseline Reproduction: Training (Top3)")
    
    EXP_NAME = "exp_m3_baseline_split"
    VALID_YEAR = 2024
    
    # 1. Load Data
    logger.info("Loading Features and Targets...")
    if not os.path.exists("data/temp_m4/M3_features.parquet") or not os.path.exists("data/temp_m4/M3_targets.parquet"):
        logger.error("Missing parquet files! Run Step 1 first.")
        return

    df_features = pd.read_parquet("data/temp_m4/M3_features.parquet")
    df_targets = pd.read_parquet("data/temp_m4/M3_targets.parquet")
    ...
    # Baseline Hyperparams (from phase_m_top3.yaml)
    # n_estimators=2000, lr=0.05, leaves=31.
    params = {
    ...
    logger.info("M3 Step 2 Completed Successfully!")
