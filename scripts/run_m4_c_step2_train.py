...
def main():
    logger.info("🚀 Starting M4-C Step 2: Training (M4-C-Top3: M4-B + Segment Stats)")
    
    EXP_NAME = "exp_m4_c_adhoc"
    VALID_YEAR = 2024
    
    # 1. Load Data
    logger.info("Loading Features and Targets...")
    if not os.path.exists("data/temp_m4/M4_C_features.parquet") or not os.path.exists("data/temp_m4/M4_C_targets.parquet"):
        logger.error("Missing parquet files! Run Step 1 first.")
        return

    df_features = pd.read_parquet("data/temp_m4/M4_C_features.parquet")
    df_targets = pd.read_parquet("data/temp_m4/M4_C_targets.parquet")
