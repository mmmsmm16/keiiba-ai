"""
Universal Annual Backtest Report Generator
==========================================
- Generates detailed ROI reports for a specified year (e.g. 2024, 2025).
- Summaries: Yearly, Monthly, Daily, Venue, Class.
- Format: Full detailed table in Markdown.
- Usage: python scripts/generate_annual_report.py --year 2024
"""
import sys
import os
import pandas as pd
...
# --- Config ---
# Paths are mostly static but OOF might vary by year if we had them.
# Currently V13 OOF is only for 2024 (Training validation) and partially 2025?
# Actually V13_OOF_PATH is `v13_oof_2024_clean.parquet`.
# If year=2025, we use live fallback or 2025 specific OOF if exists.
# If year=2024, we heavily rely on `v13_oof_2024_clean.parquet`.
...
def main():
    ...
    OUTPUT_PATH = f'reports/backtest_{target_year}_full_report.md'
    ...
    # 1. Load Data
    logger.info(f"Loading data for full year {target_year} (2016-start history window)...")
    loader = JraVanDataLoader()
    # Deep History for consistent features (2016 is robust enough for 2024/2025)
    ...
    # Filter only target year for report
    ...
    if df_target.empty:
        logger.error(f"No races found for {target_year} target range."); return

    # 3. V13 Scores (OOF Selection based on Year)
    ...
    # 2024 OOF
    if target_year == 2024:
        path = 'data/predictions/v13_oof_2024_clean.parquet'
        if os.path.exists(path): oof_files.append(path)
    # 2025 OOF
    elif target_year == 2025:
        path = 'data/predictions/v13_oof_2025_clean.parquet'
        if os.path.exists(path): oof_files.append(path)
    
    if oof_files:
        logger.info(f"Loading OOF from {oof_files}...")
        df_oof = pd.concat([pd.read_parquet(p) for p in oof_files], ignore_index=True)
        df_oof = df_oof.rename(columns={'pred_prob': 'prob_v13'})
        df_oof['race_id'] = df_oof['race_id'].astype(str)
        # Left join to preserve all target rows
    ...
    logger.info(f"Fetching Payouts for the whole year {target_year}...")
    from sqlalchemy import text
    q_hr = text("SELECT * FROM jvd_hr WHERE kaisai_nen = :y")
    with loader.engine.connect() as conn:
        df_hr = pd.read_sql(q_hr, conn, params={'y': str(target_year)})
    ...
    for rid in race_ids:
        ...
        # Breakdown Metrics
        r_inv_uma = len(partners) * 100
        r_inv_wide = len(partners) * 100
        r_ret_uma = 0
        r_ret_wide = 0
        ...
        for p in partners:
            ...
            if pair in pout['uma']: 
                r_ret_uma += pout['uma'][pair]; hits.append(f"馬連{p}")
            if pair in pout['wide']: 
                r_ret_wide += pout['wide'][pair]; hits.append(f"ワイド{p}")
        ...
        report_data.append({
            ...
            'inv_wide': r_inv_wide,
            'ret_wide': r_ret_wide,
            'inv_total': r_inv_uma + r_inv_wide,
            'ret_total': r_ret_uma + r_ret_wide,
            'hits': ",".join(hits)
        })
    ...
    with open(OUTPUT_PATH, 'w', encoding='utf-8') as f:
        f.write(f"# Annual Backtest Report: {target_year}\n\n")
        ...
        # Aggregates
        total_inv = df_rep['inv_total'].sum()
        total_ret = df_rep['ret_total'].sum()
        
        inv_u = df_rep['inv_uma'].sum(); ret_u = df_rep['ret_uma'].sum()
        inv_w = df_rep['inv_wide'].sum(); ret_w = df_rep['ret_wide'].sum()
        
        roi_total = (total_ret / total_inv * 100) if total_inv > 0 else 0
        roi_u = (ret_u / inv_u * 100) if inv_u > 0 else 0
        roi_w = (ret_w / inv_w * 100) if inv_w > 0 else 0
        
        f.write(f"## Overall Summary\n")
        f.write(f"| Type | Races | Invest | Return | ROI |\n")
        f.write(f"|------|-------|--------|--------|-----|\n")
        f.write(f"| **Combined** | {len(df_rep):,} | ¥{total_inv:,} | ¥{total_ret:,} | **{roi_total:.1f}%** |\n")
        f.write(f"| **Umaren** | {len(df_rep):,} | ¥{inv_u:,} | ¥{ret_u:,} | **{roi_u:.1f}%** |\n")
        f.write(f"| **Wide** | {len(df_rep):,} | ¥{inv_w:,} | ¥{ret_w:,} | **{roi_w:.1f}%** |\n\n")
        
        # Monthly Combined
        f.write("## Monthly Summary (Combined)\n")
        f.write("| Month | Races | Invest | Return | ROI |\n")
        f.write("|-------|-------|--------|--------|-----|\n")
        monthly = df_rep.groupby('month').agg({'rid':'count', 'inv_total':'sum', 'ret_total':'sum'})
        monthly.index = pd.to_datetime(monthly.index)
        monthly = monthly.sort_index()
        for m, r in monthly.iterrows():
            roi = (r['ret_total'] / r['inv_total'] * 100) if r['inv_total'] > 0 else 0
            f.write(f"| {m.strftime('%Y-%m')} | {r['rid']} | ¥{r['inv_total']:,} | ¥{r['ret_total']:,} | {roi:.1f}% |\n")
        f.write("\n")
        
        # Venue Breakdown
        f.write("## Venue Summary (ROI Breakdown)\n")
        f.write("| Venue | Races | ROI (Comb) | ROI (Uma) | ROI (Wide) |\n")
        f.write("|-------|-------|------------|-----------|------------|\n")
        venue = df_rep.groupby('place').agg({
            'rid':'count', 
            'inv_total':'sum', 'ret_total':'sum',
            'inv_uma':'sum', 'ret_uma':'sum',
            'inv_wide':'sum', 'ret_wide':'sum'
        })
        # Sort by total return for consistency
        for v, r in venue.sort_values('ret_total', ascending=False).iterrows():
            rc = (r['ret_total'] / r['inv_total'] * 100) if r['inv_total'] > 0 else 0
            ru = (r['ret_uma'] / r['inv_uma'] * 100) if r['inv_uma'] > 0 else 0
            rw = (r['ret_wide'] / r['inv_wide'] * 100) if r['inv_wide'] > 0 else 0
            f.write(f"| {v} | {r['rid']} | {rc:.1f}% | {ru:.1f}% | {rw:.1f}% |\n")
        f.write("\n")
        
        # Detailed Results (Expandable)
        f.write("## Detailed Prediction Log\n")
        f.write("<details>\n<summary>Click to expand all race results</summary>\n\n")
        f.write("| Date | Place | R | Class | Axis | Partners | Inv(U+W) | Ret(U+W) | Hits |\n")
        f.write("|------|-------|---|-------|------|----------|----------|----------|------|\n")
        for _, r in df_rep.iterrows():
            f.write(f"| {r['date'].strftime('%Y-%m-%d')} | {r['place']} | {r['rid'][10:12]} | {r['race_class']} | {r['axis']} | {r['partners']} | {r['inv_total']} | {r['ret_total']} | {r['hits']} |\n")
        f.write("\n</details>\n")
