...
def main():
    logger.info("🚀 Starting Phase Q Step 1 (Iteration 7): Golden Combination Feature Generation")
    
    # Feature Set (Base Q6 + Jockey Trainer Compatibility)
    FEATURE_BLOCKS = [
        ...
        'relative_stats', 'jockey_trainer_stats', # Existing simple stats
        'class_stats',
        'risk_stats',
        'course_aptitude',
        'extended_aptitude',
        'runstyle_fit',
        'jockey_trainer_compatibility' # New
    ]
    
    # We use data/features_q7 as cache dir
    CACHE_DIR = "data/features_q7"
    os.makedirs(CACHE_DIR, exist_ok=True)
    
    # 1. Copy Cache from Q6 to Q7
    logger.info("Copying Q6 cache to Q7...")
    
    q6_cache = "data/features_q6"
    if os.path.exists(q6_cache):
        import shutil
        import glob
        files = glob.glob(f"{q6_cache}/*.parquet")
        for f in files:
            bn = os.path.basename(f)
            dst = os.path.join(CACHE_DIR, bn)
            if not os.path.exists(dst):
                 shutil.copy2(f, dst)
        logger.info("  -> Copied Q6 cache.")
    else:
        logger.warning("  -> Q6 cache not found! Starting fresh.")
        
    # 2. Data Loading (Reuse Q6 Temp)
    # Target and Raw Data are same.
    TEMP_DIR = "data/temp_q7"
    ...
    clean_df[target_cols].to_parquet(f"{TEMP_DIR}/Q7_targets.parquet")
    ...
    new_block = "jockey_trainer_compatibility"
