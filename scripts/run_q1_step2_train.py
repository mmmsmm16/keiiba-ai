...
def main():
    ...
    # Standard Q1 paths (Hardcoded for Iteration 1 simplicity)
    FEAT_path = "data/temp_q1/Q1_features.parquet"
    TGT_path = "data/temp_q1/Q1_targets.parquet"
    ...
    # Expand feature blocks to actual columns?
    # FeaturePipeline generates columns, but here we need to select them.
    # The dataframe already has generated columns. 
    # We should exclude metadata columns.
    exclude_cols = ['race_id', 'horse_number', 'date', 'rank', 'target', 'year', 'time_diff']
    # If config lists specific columns, we use them. BUT config lists BLOCKS.
    # We assume all columns in df except exclude_cols are features.
