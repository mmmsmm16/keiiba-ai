...
def main():
    logger.info("🚀 Starting Phase Q Step 1 (Iteration 10): Jockey Strategy Feature Generation")
    
    # Feature Set (Base Q8 + Jockey Strategy)
    # Note: Excluding physique_training from Q9 as it was rejected
    FEATURE_BLOCKS = [
        ...
        'interval_aptitude',
        'jockey_strategy' # New for Q10
    ]
    
    # We use data/features_q10 as cache dir
    CACHE_DIR = "data/features_q10"
    os.makedirs(CACHE_DIR, exist_ok=True)
    
    # 1. Copy Cache from Q8 to Q10
    # Q8 is the approved base.
    logger.info("Copying Q8 cache to Q10...")
    ...
    # 2. Data Loading (Reuse Q8 Temp if possible, or build fresh Q10 temp)
    TEMP_DIR = "data/temp_q10"
    ...
    if 'date' in clean_df.columns:
        clean_df[target_cols].to_parquet(f"{TEMP_DIR}/Q10_targets.parquet")
    else:
    ...
    # Verify
    if 'jockey_rate_diff' not in df_features.columns:
         logger.error(f"❌ CRITICAL: {new_block} features missing!")
         raise ValueError("Feature generation failed")
         
    # Save
    out_path = f"{TEMP_DIR}/Q10_features.parquet"
    df_features.to_parquet(out_path)
    logger.info(f"Features saved to {out_path}")
    logger.info("Phase Q Step 1 (Iteration 10) Complete!")
